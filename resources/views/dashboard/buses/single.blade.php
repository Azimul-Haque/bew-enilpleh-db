                          ...
                          <form method="post" action="{{ route('dashboard.buses.update', [$district->id, $bus->id]) }}" enctype="multipart/form-data">
                            <div class="modal-body">
                                ...
                                <div id="counterFieldsEdit{{ $bus->id }}">
                                  ...
                                  @foreach($bus->buscounterdatas as $countereditdata)
                                    <div class="row mb-2 counter-group">
                                        <div class="col-md-4">
                                            <select name="countereditdata[{{ $countereditdata->id }}][buscounter_id]" class="form-control" required>
                                                <option value="">কাউন্টার নির্বাচন করুন</option>
                                                @foreach($buscounters as $buscounter)
                                                    <option value="{{ $buscounter->id }}" {{ $buscounter->id == $countereditdata->buscounter_id ? 'selected' : '' }}>{{ $buscounter->name }}</option>
                                                @endforeach
                                            </select>
                                        </div>
                                        <div class="col-md-4">
                                            <input type="text" name="countereditdata[{{ $countereditdata->id }}][address]" class="form-control" placeholder="ঠিকানা" value="{{ $countereditdata->address }}" required>
                                        </div>
                                        <div class="col-md-3">
                                            <input type="text" name="countereditdata[{{ $countereditdata->id }}][mobile]" class="form-control" placeholder="মোবাইল" value="{{ $countereditdata->mobile }}" required>
                                        </div>

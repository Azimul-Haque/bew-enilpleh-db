                ...
                @foreach ($districts->chunk(5) as $chunk)
                    <tr>
                        @foreach ($chunk as $district)
                            <td>
                              <a href="{{ route('dashboard.buses.districtwise', $district->id) }}" rel="tooltip" title="" data-original-title="{{ $district->name_bangla }}-এর বাস তালিকা দেখতে ক্লিক করুন">{{ $district->name_bangla }} <small>({{ $district }} টি বাস)</small></a>
                            </td>
                        @endforeach
                        @if ($chunk->count() < 5)

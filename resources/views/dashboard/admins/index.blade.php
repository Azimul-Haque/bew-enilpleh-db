                ...
                @foreach ($districts->chunk(5) as $chunk)
                    <tr>
                        @foreach ($chunk as $district)
                            <td>
                              <a href="{{ route('dashboard.admins.districtwise', $district->id) }}" title="{{ $district->name_bangla }} জেলার প্রশাসন কর্মকর্তা তালিকা দেখতে ক্লিক করুন" rel="tooltip" title="" data-original-title="প্রশ্ন যোগ/ হালনাগাদ করুন">{{ $district->name_bangla }}</a>
                            </td>
                        @endforeach
                        @if ($chunk->count() < 5)

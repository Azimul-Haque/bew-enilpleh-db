                    ...
                    <form class="form-inline form-group-lg" action="">
                      <select id="districtsearch" class="form-control form-control-sm" data-placeholder="জেলা ফিল্টার করুন" style="margin-right: 20px;">
                          ...
                          @foreach($districts as $district)
                            <option value="{{ $district->name_bangla }}" @isset($search) @if($district->name_bangla == $search) @endif @endisset>{{ $district->name_bangla }}-{{ $district->name }}</option>
                          @endforeach
                      </select>
    ...
    <script type="text/javascript">
        ...
        $("#districtsearch").change(function(e) {
          if($('#districtsearch').val() != '') {
            var urltocall = '{{ route('dashboard.doctors') }}' +  '/' + $('#districtsearch').val();
            location.href= urltocall;
          } else {

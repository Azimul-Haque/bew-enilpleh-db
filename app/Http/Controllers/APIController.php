...
class APIController extends Controller
{
            ...
            if($datatype == 'departmentwise') {
                $doctors = Cache::remember('doctors'. $medicalitemid . $datatype . $district_id, 30 * 24 * 60 * 60, function () use ($medicalitemid, $datatype, $district_id) {
                    ...
                    foreach($doctormedicaldepartments as $doctormedicaldepartment) {
                        ...
                        $doctormedicaldepartment->makeHidden('doctor', 'medicaldepartment_id', 'doctor_id', 'serial', 'address', 'helpline', 'weekdays', 'offdays', 'onlineserial', 'created_at', 'updated_at');
        ...
        if($softtoken == env('SOFT_TOKEN'))
        {
            $chambers = Cache::remember('doctorchambers'.$doctor_id, 30 * 24 * 60 * 60, function () use ($doctor_id) {
                ...
                foreach($chambers as $chamber) {
                    ...
                    $chamber->makeHidden('hospital', 'is_chamber', 'created_at', 'updated_at');
                    // dd($doctorstoreturn);
                }
                return $chambers;
            });
    ...
    public function storeDoctorSerial(Request $request)
    {
        ...
        $text = "Appointment\n\n" .
                ...
                "Chamber: " . $request->hospitalname . ", " . $request->upazilla . ", " . $request->district . "\n\n" .
                "SmartBD - 24/7 ই-সেবা";

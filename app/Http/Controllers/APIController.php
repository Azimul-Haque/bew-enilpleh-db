...
class APIController extends Controller
{
        ...
        if($softtoken == env('SOFT_TOKEN'))
        {
            $doctors = Cache::remember('hospitaldoctors'.$hospital_id, 30 * 24 * 60 * 60, function () use ($hospital_id) {
                ...
                foreach($doctors as $doctor) {
                    ...
                    $doctor->makeHidden('doctormedicaldepartments', 'doctorimage', 'id', 'district_id', 'upazilla_id', 'created_at', 'updated_at');
                    // dd($doctorstoreturn);
                }
                return $doctors;
            });
    ...
    public function getBloodDonorsDistrict($softtoken, $hospital_type, $district_id)
    {
        if($softtoken == env('SOFT_TOKEN'))
        {
    ...
    public function getBloodDonorsUpazilla($softtoken, $hospital_type, $district_id, $upazilla_id)
    {
        if($softtoken == env('SOFT_TOKEN'))
        {
            $blooddonors = Cache::remember('blooddonors'.$hospital_type . $district_id . $upazilla_id, 30 * 24 * 60 * 60, function () use ($hospital_type, $district_id, $upazilla_id) {
                 $blooddonors = Hospital::where('hospital_type', $hospital_type)
                             ->where('district_id', $district_id)
                             ->where('upazilla_id', $upazilla_id)
                             ->orderBy('id', 'desc')
